...
//Google翻译
let google_translation = function(data,fromLanguage,toLanguage){
  ...
  request.get({
                 'url':googleTransUrl
                ,'proxy':'http://localhost:8123'   //这里代理是防止谷歌被墙，挂的本地代理，如果可以访问请注释
                ,'User-Agent':'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.75 Safari/537.36 QQBrowser/4.1.4132.400'
              },function(error,response,body){
                if(error){
                  console.log(error);
...
let youdao_translation = function(data){
  let tmpPost = front.parse(data.raw);
  ...
  request.get({
              ...
              if(body.errorCode==0)
              {
                ...
                // console.log("OUTSIDE:====>"+json_str.translation);
              }
              if(response && response.statusCode==200){
                  // console.log(body.translation);
                  let title_array = body.translation.replace(/\s/g,"-").replace(/(.)\1+/gi,'$1').split("-");
                  let tmp_title_array = [];
                  let final_title_str = '';
                  for(var i=0;i<title_array.length;i++)
                  {
