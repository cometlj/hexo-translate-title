...
let baidu_translation_no_appid = async function (data, fromLanguage, toLanguage) {
    ...
    // let now_unixtime = Math.round(new Date().getTime() / 1000).toString();
    // let later_unixtime = (Math.round((new Date().getTime()+1) / 1000)).toString();
    let now_unixtime = moment().format('X');
    let later_unixtime = parseInt(now_unixtime)+1;
    let postHeaders = {
        // 'Accept':'*/*',
        ...
        // 'Cookie':'locale=zh; BAIDUID='+final_baiduid+' REALTIME_TRANS_SWITCH=1; FANYI_WORD_SWITCH=1; HISTORY_SWITCH=1; SOUND_SPD_SWITCH=1; SOUND_PREFER_SWITCH=1; Hm_lvt_64ecd82404c51e03dc91cb9e8c025574='+now_unixtime+'; Hm_lpvt_64ecd82404c51e03dc91cb9e8c025574='+later_unixtime+'; from_lang_often=%5B%7B%22value%22%3A%22en%22%2C%22text%22%3A%22%u82F1%u8BED%22%7D%2C%7B%22value%22%3A%22zh%22%2C%22text%22%3A%22%u4E2D%u6587%22%7D%5D; to_lang_often=%5B%7B%22value%22%3A%22zh%22%2C%22text%22%3A%22%u4E2D%u6587%22%7D%2C%7B%22value%22%3A%22en%22%2C%22text%22%3A%22%u82F1%u8BED%22%7D%5D',
